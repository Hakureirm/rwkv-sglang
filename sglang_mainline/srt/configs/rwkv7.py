...
class Rwkv7Config(PretrainedConfig):
    ...
    def mamba2_cache_params(self) -> Rwkv7CacheParams:
        ...
        # STORAGE only — the WKV kernel keeps fp32 in-register accumulation and casts
        # on the HBM load/store. At bs>=64 the fp32 state read+write is the largest
        # HBM stream, so halving it is the dominant large-batch lever, and the pool
        # allocates at this dtype (~2x slot cap). It leaves the bitwise-oracle tier,
        # so it stays default-OFF and is gated on the lossy-tier rulers (lambada +
        # compression rate) instead. conv stays fp32 always: negligible traffic,
        # and rounding it breaks greedy for zero win.
        temporal_dtype = (
            torch.float16
