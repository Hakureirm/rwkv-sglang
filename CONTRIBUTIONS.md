...
| 4. Pascal+ / AMD; PP + TP inference | ◑ | TP 2/4/8 + PP 2/4/8 + mixed, all greedy-EXACT (F0019); **verified on sglang main under cuda-graph ON (2×L4): TP=2 and PP=2 both greedy 24/24 == single-GPU + deterministic; first production throughput — tp1 peak 2,582.6 → tp2 3,026.2 (1.17×), pp2 2,288.8 (0.89×, pipeline bubbles at 1.5B) — F0036, `bench/results/tppp_l4_main.json`**. Fixed a PP+cuda-graph crash on main (v_first proxy tensor had no capture slot → KeyError; folded into PR #30115). Pascal routing guard `42fd6fa`; Pascal/AMD hardware runs pending |
| 5. w8/w4 quant: VRAM ↓, ≥ w16 speed, near Q*_K_M accuracy | ✅ speed+VRAM+acc / ◑ Q*_K_M cmp | [`bench/results/w4/`](bench/results/w4/) (+`raw/`), F0017/F0018/F0034/F0035; w4 ≥fp16 at every bsz≤32 (3090, 1.5B); **7.2B GPTQ lambada 0.7297 vs bf16 0.7425 (−1.28pt), 4.6 GB, on a real 16 GB T4**; w8 uncheatable-lossless. **sm120/Blackwell: upstream cutlass int8 GEMM absent → our own s8-wmma w8a8 kernel is the only int8 path there, beats fp16 cuBLAS 1.03–1.55× at batch≥512 (`bench/verify_w8a8.py`); on 7.2B/one 32 GB 5090 int8 serves 1.86× the concurrency (640 vs ≥344) + 13.1% higher peak (7,587 vs 6,709) than fp16 can reach — corrected 2026-07-07 from an undertested fp16 grid that stopped at 221/5,983 (F0047); original claim was F0035, `bench/results/72b/`**. Published: ModelScope `Hakureirm/rwkv7-g1-{1.5b-w8g64,1.5b-w4gptq,7.2b-w4gptq}` |
| 6. Speculative decoding (preliminary) | ◑ | viability MEASURED: 0.191B draft vs 1.5B target greedy acceptance α=0.738 (n=603) → est. ~2.0×(1.5B)/~2.7×(7.2B) bsz1 (F0029, `bench/spec_accept.py`); design ADR-0006 (recurrent chain-verify + O(1) rollback); **increment (i) FUNCTIONAL: in-engine RwkvChainWorker, 9/10 gate token-identical, 3.17 tok/round (F0031)**; exactness primitive `gemv_mb` (batch-invariant M-row GEMV, bit-identical to gemv_m1) built + gated for the verify path (commit 3cafe02); remaining: worker port to sglang main + draft/verify CUDA graphs |
| Decreed: uncheatable compression (+position curve) | ✅ | [`bench/uncheatable_eval.py`](bench/uncheatable_eval.py) + [`bench/results/uncheatable/`](bench/results/uncheatable/): 1.5B full-corpus POOLED compression fp16 **0.6085** / w8 **0.6086 (lossless, +0.0001)** / w4-GPTQ 0.6514; position curve CSVs. Current-HEAD re-run **bit-identical (0.6085, drift −0.0000)**; **w8a8 0.6161 (+0.0076), == cutlass bit-for-bit on the sm120 kernel** (`bench/results/uncheatable_full_w8a8_1.5b_5090main.json`) |
| Decreed: MATH500 avg@64 + best-bsz speed | ✅ | [`bench/math500_avg64.py`](bench/math500_avg64.py) (prompt/sampling/grader ported verbatim); 1.5B **avg@64 = 40.60%** (32k rollouts; pass@64 69.8%) — F0024; HEAD drift-gate CLOSED (identity 450/500 ≥ noise floor 446/500); **quantization on the reasoning ruler (main, RTX 5090): fp16 avg@64 0.4042 → w8a8 0.3812 (−2.3pt) → int4 GPTQ greedy 0.1560 (−24pt collapse)** — the low-variance ruler resolves what compression rate hides; the earlier greedy-based "w8a8 = fp16" was corrected (`bench/results/math500_avg64_w8a8_5090main.json`, F0034) |
