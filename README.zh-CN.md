...
| 5 | Pascal+/AMD/Intel/国产；PP+TP；zero2/3；autotune | ◑ 10 种 GPU（Turing→Blackwell）贪心精确；**TP（按 head 切）+ PP（按层切）均在真 2×L4 上贪心 24/24 精确**（tp=1/pp=1 零回归；W4/W8 暂限 tp=1；[`docs/findings/0019`](docs/findings/0019-tp-pp-parallel.md)）；⬜ Pascal/AMD/Intel 未测，训练/autotune 不在范围 |
