...
| **量化** | int8 和 int4(GPTQ),配手写 CUDA 核,int8 贪心无损(lambada 认证),量化权重已发布在 ModelScope。在 sm120/Blackwell 上——上游根本没有 int8 GEMM——我们自写的 s8-wmma 核跑通 w8a8,且在 GEMM 上快过 fp16 cuBLAS(批 ≥512 时 1.03–1.55×)。**7.2B / 单张 32GB 5090 上,int8 的并发是 fp16 的 2.32×、峰值吞吐高 16.8%**(fp16 超过 221 并发就 OOM)——详见 [BENCHMARKS §4](docs/BENCHMARKS.zh-CN.md) / [F0035](docs/findings/0035-7b-int8-concurrency-headroom.md) |
...
(fp16,但只能到 221 并发,再高就 OOM)对 **6,987 tok/s(int8,512 并发)**。bsz1 时 fp16
更快;int8 的优势是显存余量让 7.2B 扩到 2.32× 并发——完整对比见
