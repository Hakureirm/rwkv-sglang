...
| | |
|---|---|
| **正确性** | 贪心输出与 numpy fp32 参考实现逐 token 一致——0.1B / 1.5B / 7.2B(CUDA)和 Apple Silicon(MLX)都是 24/24;动态批、分块预填充、CUDA graph、张量并行/流水并行(TP 2/4/8、PP 2/4/8;TP=2 与 PP=2 已在 main 上 cuda-graph 开启时复验,贪心 24/24 == 单卡——[F0036](docs/findings/0036-pp-cudagraph-vfirst-fix.md))下同样精确 |
| **精度指标** | MATH500 avg@64(低方差钦定尺):新版 **0.4042**,与旧版 0.4060 在噪声内——无回归。压缩率:fp16 0.6085,int8 w8g64 0.6086(无损),w8a8 0.6161,int4-GPTQ 0.6514。量化在推理尺(avg@64)上:w8a8 −2.3pt、int4 −24pt——见 [§4](docs/BENCHMARKS.zh-CN.md) |
| **服务能力** | 动态批处理、分块预填充、循环状态前缀缓存(高复用负载下命中率约 98%) |
| **量化** | 两档 int8 + int4(GPTQ),全手写 CUDA。**w8g64**(仅权重):贪心无损(oracle 24/24 逐位)。**w8a8**(tensor-core):压缩率与 cutlass 一致(0.6161),MATH500 avg@64 实测 −2.3pt,换大批量/省显存。sm120/Blackwell 上——上游根本没有 int8 GEMM——我们自写 s8-wmma 核跑通 w8a8 且 GEMM 快过 fp16 cuBLAS(批 ≥512 时 1.03–1.55×)。**7.2B / 单张 32GB 5090 上 int8 并发 1.86×、峰值高 13.1%**(fp16 真实上限是 ≥344 并发,2026-07-07 修正了此前测试网格不足导致的 221 误报)——详见 [BENCHMARKS §4](docs/BENCHMARKS.zh-CN.md) / [F0047](docs/findings/0047-fp16-72b-concurrency-correction.md) |
...
**7.2B,单张 RTX 5090(32GB):** 单请求 **142.8 tok/s**(fp16)——megakernel 旗舰阶梯(2026-07-21,F0066c),完整逐步过程见 [BENCHMARKS §7a-flagship](docs/BENCHMARKS.zh-CN.md)。峰值服务:**6,709 tok/s
(fp16 @ c320,安全运行到 ≥344 并发)** 对 **7,587 tok/s(int8,640 并发)**。bsz1 时 fp16
更快;int8 的优势是显存余量让 7.2B 扩到 1.86× 并发——完整对比(以及对此前发布的
5,983/221 因并发网格测试不足而误报的修正)见 [BENCHMARKS §4](docs/BENCHMARKS.zh-CN.md)。
...
- **与 BlinkDL 的 Albatross 对比**(官方速度参照;注意它是一个纯测速程序,没有请求调度和
  ...
  **0.9908 倍**。T4 这类老卡上**stock** Albatross 核编译不过(用了 sm80+ 的 `cp.async`——这是可移除的限制,BlinkDL 说改核后能在 T4 跑),所以开箱即用只有我们的栈能服务 T4。逐卡数据:[`albatross_fleet_10cards.json`](bench/results/albatross_fleet_10cards.json)。
