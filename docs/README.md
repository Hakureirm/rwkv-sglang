...
| [F0079](findings/0079-large-batch-is-gemm-bound.md) | Profiled the c=320 step for the first time (every prior profile was bsz1): **58% of it is one cutlass GEMM**, overlapped transitions fall 95.6% -> 19.8%, and the achieved GEMM rate is ~155 TFLOP/s against a much higher card peak. Three consequences — the megakernel/PDL ladder is a bsz1 lever and should not be read as a serving-wide number; the large-batch path is stock cuBLAS plus our glue, with none of our hand-written kernels participating; and the trace shows `cutlass_80_*` tiles on Blackwell plus 132 splitK reductions per step. Names the next lever without claiming it is recoverable |
