...
| [F0077](findings/0077-spec-decode-draft-desync-fix.md) | RWKV_SPEC completion push — the accept-length collapse was a one-token rollback bug (draft desynced on every partial accept, invisible to the correctness gate; accept 1.2→2.5-3.5 after the fix, matching the independently re-measured alpha), plus a hand-rolled TARGET_VERIFY CUDA graph (1.5B round 35.4→12.5 ms) and a prefill merge-safety fix; gate 10/10 at 1.5B AND 7.2B, net still 0.78x/0.85x median — only high-accept workloads cross 1x (7.2B math 1.04x) |
