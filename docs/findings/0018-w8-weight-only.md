...
doc_kind: finding
finding_id: F0018
title: "Hand-written weight-only int8 (w8a16): greedy-EXACT 24/24 (lossless in practice), faster than fp16 at every bsz≤8 (1.06–1.37× e2e), and JIT-runs on every arch — unlike cutlass w8a8 (sm80–90 only)"
...
- `gemv_w8_m1` (M=1) + `gemm_w8_small` (2≤M≤8, one weight-word feeds all M rows, every row
  BIT-identical to the M=1 kernel — torch.equal-verified) + `dequant_w8` (M>8 → cuBLAS).
...
Standalone kernels vs fp16 cuBLAS (`bench/verify_w8.py`): **1.17–2.29× at every M∈{1,2,4,8}**,
every shape (2048², 4096², 4096×14336); numerics 2.1e-4 vs the dequant reference.
...
| 32 | 3872.6 | 2500.0 | 0.65× (dequant→cuBLAS fallback; a w8 tensor-core tile kernel like `gemm_w4_tc` is the known follow-up) |
...
|---|---|---|---|
| **w8a16 (this)** | **greedy-EXACT** | bsz≤8: 1.06–1.37× fp16 | **all** (JIT per-arch) |
