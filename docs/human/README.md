...
| 量化 | int8 追平 albatross-fp16 且省 46% 权重；手写 int4 bsz1 **快 1.54×** |
| 覆盖 | 8 种 GPU 架构（Turing→Hopper）全部跑通 |
...
→ bf16 全 8 卡精确；int4 全 8 卡可跑且更快（**含 Turing 老架构**，核不依赖 cp.async）。
...
flowchart LR
    ...
    W --> I4["int4 (手写 GEMV)<br/>权重 ~4×↓<br/>bsz1 解码 1.54× 更快"]
...
RWKV_W4=1 ...（配合 int4 checkpoint）        # 4-bit：省显存 + bsz1 更快
```
