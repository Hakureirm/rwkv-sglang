...
class ModelRunnerKVCacheMixin:
    ...
    def profile_max_num_token(self: ModelRunner, pre_model_load_memory: int):
        ...
        if cell_size == 0:
            # All-linear models (e.g. RWKV-7) have zero full-attention layers, so the
            # per-token full-attn KV cost is 0. Token capacity is then not bounded by
            # full-attn KV memory but by the mamba/linear-state pool concurrency.
            # Bound max_total_num_tokens by max_mamba_cache_size * context_len, clamped
            # to keep the token allocator index tensor reasonable.
            return min(
                self.server_args.max_mamba_cache_size * self.model_config.context_len,
                1 << 20,
            )
